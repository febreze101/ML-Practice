...
def run(is_training=True, render=False):
    ...
    if (is_training):
         q = np.zeros((len(pos_space) + 1, len(vel_space) + 1, len(ang_space) + 1, len(ang_vel_space) + 1, env.action_space.n)) # init a 11x11x11x11x3 array
    else:
    ...
    while(True):
        ...
        # learning_rate_a = max(learning_rate_a - learning_rate_a_decay, 0.00001)
